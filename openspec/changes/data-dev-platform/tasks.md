...
- [ ] 1.5 评审"运行时变量目录"YAML(`${dt}/${date}/${month}/${dt-N}/${date-N}/${hr}`),作为 Frontend / Backend / driver 三方共享 fixture
...
- [ ] 3.1 把"运行时变量目录"YAML 引入 driver,实现 `${dt}/${date}/${month}/${dt-N}/${date-N}/${hr}` 渲染(specs/runtime-variable-rendering)。`${dt-N}/${date-N}` 用独立 pattern 优先匹配(连字符不在普通变量名字符集);`${hr}` 缺 `--biz-hour` 时 fail-fast
...
- [ ] 3.4 默认严格模式:未定义变量 fail-fast,错误信息列出全部未定义变量
